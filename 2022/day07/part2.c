...
static int
traverse_for_solution(node_t* node, int size_to_cut, int* size_found) {
  static int scope = 0;
  node_t* itr = node; 
  int ret = 0;
  while(itr) {
    ...
    if (itr->type == NODE_TYPE_FILE) {
      ...
      scope+=2;
      printf("dir  (%10s)\n", itr->name);
      int sum = traverse_for_solution(itr->head, size_to_cut, size_found);
      int found = 0;
      if (sum > size_to_cut) {
        if (sum < *size_found || *size_found == -1) {
          *size_found = sum;
          found = 1;
        }
      }
...
#define push_node(head, node) (node)->next = (head); (head) = (node);
  ...
  int total_size = node_size(nodes);
  printf("total_size: %d\n", total_size);
  int size_to_cut = total_size - 40000000;
  printf("size_to_cut: %d\n", size_to_cut);

  printf( "=== Checking contents ===\n");
  int size_found = -1;
  traverse_for_solution(nodes, size_to_cut, &size_found );
  printf("size found: %d\n", size_found);
